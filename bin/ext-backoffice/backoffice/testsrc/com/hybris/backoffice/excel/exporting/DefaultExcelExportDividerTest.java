...
public class DefaultExcelExportDividerTest
{
	...
	@Test
	public void shouldAvoidPKAttributes()
	{
		// given
		...
		final List<AttributeDescriptorModel> ads = generateAttributeDescriptorMocks(Lists.newArrayList(//
				...
				new Descriptor(false, false, true, ItemModel.PK, ItemModel.PK)//
		));
		given(cp.getUniqueKeyAttributes()).willReturn(ads);
	...
	@Test
	public void shouldCheckPermissions()
	{
		// given
		final ComposedTypeModel cp = mock(ComposedTypeModel.class);
		final List<AttributeDescriptorModel> ads = generateAttributeDescriptorMocks(Lists.newArrayList(//
				new Descriptor(false, false, true, "Name", "qualifier")//
		));
		given(cp.getUniqueKeyAttributes()).willReturn(ads);
		when(permissionCRUDService.canReadAttribute(any(AttributeDescriptorModel.class))).thenReturn(false);

		// when
		final Set<SelectedAttribute> result = divider.getMissingRequiredAndUniqueAttributes(cp, new HashSet<>());

		// then
		soft.assertThat(result.size()).isEqualTo(0);
	}
