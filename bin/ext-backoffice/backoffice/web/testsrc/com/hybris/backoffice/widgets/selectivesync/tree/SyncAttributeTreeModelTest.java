...
public class SyncAttributeTreeModelTest
{
	...
	*/
	...
	public void setUp()
	{
		...
		attributes.add(TreeModelTestUtils.createAndAddSyncAttribute(rootType, "attribute1"));
		attributes.add(TreeModelTestUtils.createAndAddSyncAttribute(rootType, "attribute2"));
		attributes.add(TreeModelTestUtils.createAndAddSyncAttribute(node1, "attribute3"));
		attributes.add(TreeModelTestUtils.createAndAddSyncAttribute(node1, "attribute4"));
		attributes.add(TreeModelTestUtils.createAndAddSyncAttribute(node2, "attribute5"));
		attributes.add(TreeModelTestUtils.createAndAddSyncAttribute(node2, "attribute5"));//second attribute with same name on this node
		attributes.add(TreeModelTestUtils.createAndAddSyncAttribute(node1, "attribute5"));//same attribute name but different node
	...
	{
		TreeModel<SyncTypeAttributeDataTreeNode> filtered = treeModel
				.filter(new FilterContext("node1", true, true));
		...
		filtered = treeModel.filter(new FilterContext("attribute1", true, true));
		assertThat(filtered.getChildCount(SyncTypeAttributeDataTreeNode.createTypeNode(rootType))).isEqualTo(1);
		assertThat(filtered.getChildCount(SyncTypeAttributeDataTreeNode.createTypeNode(node1))).isEqualTo(0);
		assertThat(filtered.getChildCount(SyncTypeAttributeDataTreeNode.createTypeNode(node2))).isEqualTo(0);

		filtered = treeModel.filter(new FilterContext("attribute3", true, true));
		assertThat(filtered.getChildCount(SyncTypeAttributeDataTreeNode.createTypeNode(rootType))).isEqualTo(1);
		assertThat(filtered.getChildCount(SyncTypeAttributeDataTreeNode.createTypeNode(node1))).isEqualTo(1);
		assertThat(filtered.getChildCount(SyncTypeAttributeDataTreeNode.createTypeNode(node2))).isEqualTo(0);

		filtered = treeModel.filter(new FilterContext("attribute5", true, true));
		assertThat(filtered.getChildCount(SyncTypeAttributeDataTreeNode.createTypeNode(rootType))).isEqualTo(1);
	...
	private void assertZeroChildCount(final TreeModel<SyncTypeAttributeDataTreeNode> treeModel,
			final ComposedTypeModel... composedTypeModels)
	{
		for (final ComposedTypeModel model : composedTypeModels)
		{
			assertThat(treeModel.getChildCount(SyncTypeAttributeDataTreeNode.createTypeNode(model))).isEqualTo(0);
		}
	}
}
