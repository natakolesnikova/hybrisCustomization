...
public class PageModelToDataRenderingPopulator implements Populator<AbstractPageModel, AbstractPageData>
{
	...
	public void populate(final AbstractPageModel sourceModel, final AbstractPageData targetData)
	{
		...
		// Slots
		targetData.setContentSlots(getCmsPageService().getContentSlotsForPage(sourceModel).stream().filter(this::isSlotActive)
				.map(getContentSlotModelToDataRenderingConverter()::convert).collect(Collectors.toList()));
	}
