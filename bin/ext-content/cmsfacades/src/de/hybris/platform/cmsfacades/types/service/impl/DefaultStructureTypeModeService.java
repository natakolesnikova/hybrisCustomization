...
public class DefaultStructureTypeModeService implements StructureTypeModeService, InitializingBean
{
	...
	private Supplier<Map<String, Map<StructureTypeMode, ComponentTypeStructure>>> componentTypeDataSupplierMap = initializeInternalStructureMap(DEFAULT_EXPIRATION_TIME);

	@Override
	public ComponentTypeStructure getComponentTypeByCodeAndMode(final String typeCode,
			final StructureTypeMode structureTypeMode)
	{
	...
	protected Supplier<Map<String, Map<StructureTypeMode, ComponentTypeStructure>>> initializeComponentTypeDataSupplier()
	{
		return () ->
		{
			...
			componentTypeDataMap = getComponentTypeStructureService()
	...
	{
		final Map<StructureTypeMode, ComponentTypeStructure> structureTypeModeMap = new HashMap<>();
	 ...
	 * @param mode the current mode being inspected.
	 * @param typeStructure the typeStructure with the attributes to be inspected. 
	 */
	protected void augmentAttributePopulators(final StructureTypeMode mode, final ComponentTypeStructure typeStructure)
	{
		typeStructure.getAttributes().stream().forEach(
				attribute -> 
					getAttributeDescriptor(typeStructure.getTypecode(), attribute.getQualifier()) //
						.ifPresent(attributeDescriptor ->
							attribute
									.getPopulators() //
	...
	/**
	 * Convert the {@link ComponentTypeStructure} into {@link ComponentTypeData}. 
	 * It creates a new object as the image of the DEFAULT, if not empty, or the BASE, which is never null. 
	 * @param mode
	 *           the mode for this current type structure view.
	 *@param baseTypeStructure
	 *           the BASE type structure
	 *@para defaultTypeStructureOptional optional object
	 * 			 the DEFAULT type structure
	 ...
	 * @param typeCode the type code we are want to apply the mode into. 
	 * @param mode the mode definition
	 * @param baseTypeStructure the base type Structure that is mode-agnostic. 
	 * @param attributes the pre-built set of attributes defined by the base. 
	 * @param attributeOrder the final attribute order
	 */
			...
			final Set<String> attributeOrder)
	{
		...
		final Deque<StructureTypeModeAttributeFilter> structureTypeModeAttributeFilterList = getStructureTypeModeData(typeCode, mode);
	 ...
	 * @param typeCode the type code that defines a mode. 
	 * @param mode the structure mode required for this type
	 ...
	 * @param typeCode the current type code possibly owning the mode definition 
	 * @param mode the mode definition
	 * @param structureModeDataList the final list of modes collected. 
	 */
			...
			final Deque<StructureTypeModeAttributeFilter> structureModeDataList)
	{
		...
		final Deque<StructureTypeModeAttributeFilter> internalDeque = 
				new LinkedList<>(getStructureTypeModeAttributeFilterProvider().getStructureTypeModeAttributeFilters(typeCode, mode));
		
		internalDeque.descendingIterator().forEachRemaining(structureTypeModeData -> 
			structureModeDataList.add(structureTypeModeData)	
		);
	...
	/**
	 * Sets the attribute's position to be equal to the position of the attribute in the ordered list, 
	 * or to be Integer.MAX_VALUE, if the attribute is not present in the ordered list.
	 * @param attribute the ComponentTypeAttributeStructure we want to set the position
	 * @param attributeOrder the ordered list
	 */
	protected void setAttributePosition(final ComponentTypeAttributeStructure attribute, Set<String> attributeOrder)
	{
		final List<String> order = new ArrayList<>(attributeOrder);
		
		final Integer position = order.indexOf(attribute.getQualifier());
		
		((DefaultComponentTypeAttributeStructure) attribute)
				.setPosition(position == -1 ? Integer.MAX_VALUE : position);
	}
	...
	{
		this.componentTypeDataSupplierMap = initializeInternalStructureMap(getConfigurationService().getConfiguration().getLong(
				TYPE_CACHE_EXPIRATION, DEFAULT_EXPIRATION_TIME));
	}
