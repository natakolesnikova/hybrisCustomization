...
/**
 * Validate fields of {@Link UserGroupRestrictionData} for create operation
 * 
 * @deprecated since 6.6
 */
@HybrisDeprecation(sinceVersion = "6.6")
@Deprecated
public class CreateUserGroupRestrictionValidator implements Validator
{
	...
	{
		return UserGroupRestrictionData.class.isAssignableFrom(aClass);
	}

	@Override
	public void validate(final Object obj, final Errors errors)
	{
		final UserGroupRestrictionData target = (UserGroupRestrictionData) obj;
		ValidationUtils.rejectIfEmpty(errors, USERGROUPS, FIELD_REQUIRED);

		if (Objects.nonNull(target.getUserGroups()))
		{
			if (target.getUserGroups().isEmpty())
			{
				errors.rejectValue(USERGROUPS, FIELD_MIN_VIOLATED);
			}

			target.getUserGroups().stream()
