...
public class DefaultValidationErrorsProvider implements ValidationErrorsProvider
{
	...
	public ValidationErrors getCurrentValidationErrors()
	{
		...
		try 
		{
			...
			if (value == null)
			{
				throw new IllegalStateException("There is no current validation error context. Please Initialize with #initializeValidationErrors before using this method.");
			}
			else
			{
	...
	@Override
	public void collectValidationErrors(ValidationException e, Optional<String> language, Optional<Integer> position)
	{
	 ...
	 */
	protected Deque<ValidationErrors> getWrappedStack(Object rawValue)
	{
		...
		throw new IllegalStateException("Session object for SESSION_VALIDATION_ERRORS_OBJ should hold a reference of AtomicReference object.");
	}
