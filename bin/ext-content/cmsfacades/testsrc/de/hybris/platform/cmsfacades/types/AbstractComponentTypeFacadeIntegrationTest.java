...
public class AbstractComponentTypeFacadeIntegrationTest
{
	...
	public void setup()
	{
		addTruthyType(ContentPageModel._TYPECODE, StructureTypeCategory.PAGE.name(), Arrays.asList(
				newAttribute(ContentPageModel.UID, SHORT_STRING),
				newAttribute(ContentPageModel.NAME, SHORT_STRING),
				newAttribute(ContentPageModel.TITLE, SHORT_STRING),
				newAttribute(ContentPageModel.CREATIONTIME, DATE),
				newAttribute(ContentPageModel.MODIFIEDTIME, DATE),
				newAttribute(ContentPageModel.LABEL, SHORT_STRING)
				));
		addTruthyType(ProductPageModel._TYPECODE, StructureTypeCategory.PAGE.name(), Arrays.asList(
				newAttribute(ProductPageModel.UID, SHORT_STRING),
				newAttribute(ProductPageModel.NAME, SHORT_STRING),
				newAttribute(ProductPageModel.TITLE, SHORT_STRING),
				newAttribute(ProductPageModel.CREATIONTIME, DATE),
				newAttribute(ProductPageModel.MODIFIEDTIME, DATE)
				));
		addTruthyType(CategoryPageModel._TYPECODE, StructureTypeCategory.PAGE.name(), Arrays.asList(
				newAttribute(CategoryPageModel.UID, SHORT_STRING),
				newAttribute(CategoryPageModel.NAME, SHORT_STRING),
				newAttribute(CategoryPageModel.TITLE, SHORT_STRING),
				newAttribute(CategoryPageModel.CREATIONTIME, DATE),
		...
		addTruthyType(CMSTimeRestrictionModel._TYPECODE, StructureTypeCategory.RESTRICTION.name(), Arrays.asList(
				newAttribute(CMSTimeRestrictionModel.UID, SHORT_STRING),
				newAttribute(CMSTimeRestrictionModel.NAME, SHORT_STRING),
				newAttribute(CMSTimeRestrictionModel.DESCRIPTION, SHORT_STRING),
				newAttribute(CMSTimeRestrictionModel.ACTIVEFROM, DATE),
				newAttribute(CMSTimeRestrictionModel.ACTIVEUNTIL, DATE)
				));
		addTruthyType(CMSCategoryRestrictionModel._TYPECODE, StructureTypeCategory.RESTRICTION.name(), Arrays.asList(
				newAttribute(CMSCategoryRestrictionModel.UID, SHORT_STRING),
		...
		addTruthyType(PreviewDataModel._TYPECODE, StructureTypeCategory.PREVIEW.name(), Arrays.asList(
				newAttribute(PreviewDataModel.PREVIEWCATALOG, EDITABLE_DROPDOWN),
				newAttribute(PreviewDataModel.LANGUAGE, EDITABLE_DROPDOWN),
				newAttribute(PreviewDataModel.TIME, DATE)
				));
	}
	...
	protected void assertAvailableComponentTypeData(final ComponentTypeData componentTypeData)
	{
		...
		assertThat("Type Code is different than expected for [" + componentTypeData.getCode() + "]",
				componentTypeData.getCode(), is(bluePrint.getCode()));
		assertThat("Category is different than expected for [" + componentTypeData.getCode() + "]",componentTypeData.getCategory(), is(bluePrint.getCategory()));
		assertThat("Name is different than expected for [" + componentTypeData.getCode() + "]", componentTypeData.getName(), is(bluePrint.getName()));
		assertThat("I18n is different than expected for [" + componentTypeData.getCode() + "]", componentTypeData.getI18nKey(), equalTo(bluePrint.getI18nKey()));
		componentTypeData.getAttributes().stream().forEach(componentTypeAttributeData -> {
			...
			.ifPresent(attrBluePrint -> {
				assertThat("Attribute type [" + componentTypeAttributeData.getQualifier() + "] is different than expected for [" + componentTypeData.getCode() + "]",
	...
	protected void assertPageTypeAttributes(final ComponentTypeData pageType)
	{
		...
		final int count = (int) pageType.getAttributes().stream()
				.filter(attribute -> expectedQualifiers.contains(attribute.getQualifier()))
				.count();
