...
public class DefaultItemSynchronizationFacadeTest
{
	...
	public void setup()
	{
		...
		when(synchronizationStatusService.getSynchronizationItemStatus(syncRequestData, contentPage)).thenReturn(syncPageDetailsData);
	...
	public void testWhenItemCollectorRegistriesReturnEmpty_shouldNotPopulateMoreThanOneLevel()
	{
		...
		final SyncItemStatusData synchronizationItemStatus = synchronizationStatusFacade.getSynchronizationItemStatus(
				syncRequestData, pageItemData);
	...
	public void testWhenCollectorReturnsSlots_shouldPopulateSelectedDependencies()
	{
		...
		verify(sharedItemCollectorRegistry).getItemCollector(contentPage);
		verify(dependentItemCollectorRegistry).getItemCollector(contentPage);
		verify(synchronizationItemStatus).setSelectedDependencies(asList(syncSlot1StatusData, syncSlot2StatusData, syncSlot3StatusData, syncSlot4StatusData));
		verify(synchronizationItemStatus).setSharedDependencies(newArrayList());
		verify(synchronizationItemStatus).setUnavailableDependencies(newArrayList());
	}
	...
	public void testWhenCollectorReturnsSlots_shouldPopulateSharedDependencies()
	{
		...
		verify(synchronizationItemStatus).setSharedDependencies(newArrayList(syncSlot1StatusData, syncSlot2StatusData, syncSlot3StatusData, syncSlot4StatusData));
		verify(synchronizationItemStatus).setUnavailableDependencies(newArrayList());
	}
	...
	public void performItemSynchronizationWithConfigWillDelegateToSyncServiceWithConfig()
	{
		...
		verify(synchronizationStatusService, times(1)).performItemSynchronization(eq(syncRequestData), itemListCaptor.capture(), eq(syncConfig));
	...
	public void performItemSynchronizationWithoutConfigWillDelegateToSyncServiceWithDefaultConfig()
	{
		...
		verify(synchronizationStatusService, times(1)).performItemSynchronization(eq(syncRequestData), itemListCaptor.capture(), eq(defaultConfig));
