...
public class CreateUserGroupRestrictionValidatorTest
{
	@InjectMocks
	private CreateUserGroupRestrictionValidator validator;
	...
	private final List<String> USERGROUP_KEYS = Arrays.asList(
	...
	class UserGroupRestrictionDataSubclass extends UserGroupRestrictionData
	{
	...
	public void setUp()
	{
		...
		USERGROUP_KEYS.forEach(userGroupKey ->
	...
	{
		Assert.assertThat("CreateUserGroupRestrictionValidator should support UserGroupRestrictionData class or its subclasses",
	...
	{
		Assert.assertThat("CreateUserGroupRestrictionValidator should support UserGroupRestrictionData class",
				validator.supports(UserGroupRestrictionData.class), Matchers.is(true));
	}

	@Test
	public void knownSubclassIsSupported()
	{
		Assert.assertThat("CreateUserGroupRestrictionValidator should support UserGroupRestrictionData subclass",
				validator.supports(UserGroupRestrictionDataSubclass.class), Matchers.is(true));
	}
	...
	{
		//prepare
		data.setUserGroups(USERGROUP_KEYS);
		when(errors.getFieldValue(USERGROUPS)).thenReturn("fake-data");
		...
		assertThat("CreateUserGroupRestrictionValidator errors object should contain 2 USERGROUPS fields",
				fieldNames, contains(Arrays.asList(USERGROUPS, USERGROUPS).toArray()));

		assertThat("CreateUserGroupRestrictionValidator errors object should contain 2 FIELD_REQUIRED codes",
				errorCodes, contains(Arrays.asList(FIELD_DOES_NOT_EXIST, FIELD_DOES_NOT_EXIST).toArray()));

		assertThat("CreateUserGroupRestrictionValidator errors object should contain 2 VALUE objects",
