...
@IntegrationTest
public class DefaultStructureTypeModeServiceIntegrationTest extends ServicelayerTest
{
	@Resource
	...
	public void testCMSLinkComponentByModeDEFAULT_shouldNotReturnExcludedAttributes() throws ComponentTypeNotFoundException
	{
		final ComponentTypeData componentTypeData = componentTypeFacade.getComponentTypeByCodeAndMode(
				CMSLinkComponentModel._TYPECODE, StructureTypeMode.DEFAULT.name());

		assertThat(componentTypeData.getAttributes()
						.stream()
						.map(ComponentTypeAttributeData::getQualifier)
						.collect(Collectors.toList()),
				not(hasItems(CONTENTPAGE, PRODUCT, TARGET, CATEGORY, URL, EXTERNAL, CONTENTPAGELABELORID, PRODUCTCODE, CATEGORYCODE, NAVIGATIONNODES, CONTENTPAGEPOS, PRODUCTPOS, CATEGORYPOS)));
	}
	
	@Test
	public void testCMSTimeRestrictionByModeDEFAULT_shouldNotReturnExcludedAttributesFromAbstractRestriction() throws ComponentTypeNotFoundException
	{
		final ComponentTypeData componentTypeData = componentTypeFacade.getComponentTypeByCodeAndMode(
	...
	@Test
	public void testProductCarouselComponentModelByModeDEFAULT_shouldNotReturnExcludedAttributes() throws ComponentTypeNotFoundException
	{
		final ComponentTypeData componentTypeData = componentTypeFacade.getComponentTypeByCodeAndMode(
				ProductCarouselComponentModel._TYPECODE, StructureTypeMode.DEFAULT.name());
