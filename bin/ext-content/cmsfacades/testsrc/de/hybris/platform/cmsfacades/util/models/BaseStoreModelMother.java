...
public class BaseStoreModelMother extends AbstractModelMother<BaseStoreModel>
{
	...
	{
		CatalogModel[] catalogModels = asList(catalogVersionModels).stream().map(e -> e.getCatalog()).collect(toList()).toArray(new CatalogModel[]{});
		...
		return getFromCollectionOrSaveAndReturn(() -> getBaseStoreDao().findBaseStoresByUid(UID_NORTH_AMERICA),
				() -> BaseStoreModelBuilder.aModel().withCatalogs(catalogModels)
						...
						.withLanguages(getLanguageModelMother().createEnglish()).withNet(Boolean.FALSE).withPaymentProvider("Mockup")
						.withSubmitOrderProcessCode("order-process").withUid(UID_NORTH_AMERICA)
						.withDeliveryModes(getDeliveryModeModelMother().Pickup(), getDeliveryModeModelMother().Regular()).build());
	}
