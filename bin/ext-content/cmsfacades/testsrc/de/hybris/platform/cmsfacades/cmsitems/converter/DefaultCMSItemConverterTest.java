...
{
	private final String QUALIFIER_1 = "Some_qualifier_1";
	private final String QUALIFIER_2 = "Some_qualifier_2";
	private final String QUALIFIER_3 = "Some_qualifier_3";

	private final String ATTR_1_VALUE = "some_attr_1_value";
	private final String ATTR_2_VALUE = "some_attr_2_value";
	private final String ATTR_3_VALUE = "some_attr_3_value";

	private ArgumentCaptor<String> qualifierCaptor = ArgumentCaptor.forClass(String.class);
	private ArgumentCaptor<Object> valueCaptor = ArgumentCaptor.forClass(Object.class);
	...
	public void isDynamicAttributeShouldReturnTrueForDynamicAttributes()
	{
		...
		// WHEN
		boolean result = cmsItemConverter.isDynamicAttribute(attributeDescriptorModel1);
	...
	{
		// GIVEN
		Map<String, Object> map = buildTestDataMap();
		setUpDataToModelTest(map);

		// WHEN
		ItemModel result = cmsItemConverter.convertAndValidate(map, composedType);
	...
	{
		String itemType = (attr instanceof RelationDescriptorModel) ?
				RelationDescriptorModel._TYPECODE : AttributeDescriptorModel._TYPECODE;
	...
	protected void setUpDataToModelTest(final Map<String, Object> testMap)
	{
		...
		doReturn(itemModel)
				.when(cmsItemConverter).getItemModelFromRepresentation(testMap);
		doNothing()
				.when(cmsItemConverter).validate(any(), any(), any());
		doNothing()
				.when(cmsItemConverter).collectValidationErrors(any(), any(), any());
		...
		doAnswer(invocation -> (Function<AttributeDescriptorModel, Optional<Object>>) attr -> Optional.of(invocation.getArgumentAt(0, Object.class)))
				.when(cmsItemConverter).convertRepresentationToAttributeValue(any());
	}
	...
	protected <T extends RuntimeException> void makeAttributeThrowExceptionWhenValidated(final AttributeDescriptorModel attr, final Class<T> expectedExceptionClass)
	{
		doThrow(expectedExceptionClass)
				.when(cmsItemConverter).validate(any(), eq(attr), any());
	}

	protected void assertValuesFromMapWereSetInModel(final Map<String, Object> originalMap, final String... expectedValues)
	{
		verify(modelService, times(expectedValues.length)).setAttributeValue(any(ItemModel.class), qualifierCaptor.capture(), valueCaptor.capture());
		for(int i = 0; i < expectedValues.length; i++)
		{
			String expectedKey = expectedValues[i];
